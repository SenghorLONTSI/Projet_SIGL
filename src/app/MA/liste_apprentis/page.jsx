...
export default function MesApprentisPage() {
  ...
  const filteredApprentis = useMemo(() => {
    ...
    if (filterTP === "with") {
      list = list.filter((a) => !!a.tp);
    } else if (filterTP === "without") {
      list = list.filter((a) => !a.tp);
    }
    ...
    return list;
  }, [apprentis, search, filterTP]);

  const total = apprentis.length;
  const withTP = apprentis.filter((a) => !!a.tp).length;
  const withoutTP = total - withTP;
  ...
  return (
    <div className="max-w-6xl mx-auto mt-10 px-4 space-y-6">
      ...
      <header className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Liste de vos apprentis et des tuteurs pédagogiques (TP) associés.
          </p>
        </div>
      </header>
      ...
      {!loading && !error && total > 0 && (
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative w-full sm:w-2/3">
            <input
              type="text"
              placeholder="Rechercher par nom, email, TP..."
              value={search}
      ...
      {/* États simples */}
      {loading && <p>Chargement des apprentis...</p>}
                ...
                <div className="rounded-md border border-slate-100 bg-slate-50/70 px-3 py-2">
                  ...
                  {a.tp ? (
                    ...
                    <p className="mt-2 text-[11px] text-slate-500">
                      Aucun TP n&#39;est encore associé à cet apprenti.
                    </p>
                  )}
                </div>
              </CardContent>
